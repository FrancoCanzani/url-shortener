...
export function Features() {
  return (
    <div className='px-4 pb-16 max-w-[52rem] mx-auto grid grid-cols-1 gap-2 lg:gap-4 sm:grid-cols-2 lg:grid-cols-3 max-h-[33rem] overflow-hidden'>
      <Feature
        svg={
          <svg
        ...
        text='optimize your campaigns with analytics for each shortened link'
      />
      <Feature
        svg={
          <svg
      ...
      <Feature
        ...
        title='optimize your campaigns'
        text='manageable and aesthetically pleasing in social posts,'
      />
    </div>
  );
}
...
export function Feature({
  ...
  return (
    <figure className='flex hover:scale-105 transition-all ease-linear shadow-sm flex-col bg-gray-100 rounded-lg p-3'>
      <span className='bg-gray-50 rounded-lg p-2 w-fit mb-3'>{svg}</span>
      <h3 className='text-lg font-semibold mb-1'>{title}</h3>
      <p className='capitalize font-normal text-sm'>{text}</p>
    </figure>
  );
}
